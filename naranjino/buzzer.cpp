...
unsigned long ON_CYCLES ;
unsigned long OFF_CYCLES ;
...
void buzzer_time(float on,float off)
{
  // These are the number of seconds the buzzer will stay on/off alternately
  ON_CYCLES = BUZZER_FREQ * on;
  OFF_CYCLES = BUZZER_FREQ * off;
}
...
void buzzer_setup()
{
  buzzer_time(BUZZER_ON_TIME,BUZZER_OFF_TIME);
